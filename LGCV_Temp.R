...
sam<-sample(unique(clinic_cnts$pseudo_personid),500)
...
models<-c(FLXMRglm(family="poisson"),FLXMRziglm(family = "poisson"),nb=FLXMRnegbin())#set models to test 
...
nb_model1<-stepFlexmix(n~(Mon_Flag+prodline)|pseudo_personid,data=sample_df,model=nb,k=1:2)
getModel(nb_model1)

nb_model1new<-stepFlexmix(n~prodline+Mon_Flag|pseudo_personid,data=sample_df,model=nb,k=1:2)
getModel(nb_model1new)@components

final_model@components

class_ass<-cbind(sample_df,getModel(nb_model1new)@cluster)
class_ass%>%
  distinct(pseudo_personid,`getModel(nb_model1new)@cluster`)%>%
