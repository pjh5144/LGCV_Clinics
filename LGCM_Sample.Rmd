...
ggplot(class_sum%>%
         filter(prodline %in% c("IMSUB","MH","ORTHO","OTHER","PC","SURG")),aes(x=Mon_Flag,y=mean))+
  geom_point(aes(color=cluster))+facet_wrap(~prodline)+
  geom_line(aes(color=cluster))+ggtitle("Average Number of Encounters per Month by Cluster")+
  theme(plot.title = element_text(hjust = 0.5))+xlab("Month Following Injury")+ylab("Average # Encounters")
...
pts_final%>%
  group_by(cluster)%>%
  summarise(min=round(min(age),2),mean=round(mean(age),2),max=round(max(age),1))%>%
  kable()%>%kable_styling(full_width=F)
